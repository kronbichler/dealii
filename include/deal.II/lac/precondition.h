...
public:
  ...
  /**
   * Compute the action of the preconditioner on the given source vector,
   * returning the norm of the residual used in the last iteration step. Since
   * the Chebyshev iteration uses the residual for another update step, the
   * final residual norm is different, typically smaller, than the value
   * reported by this function.
   */
  double
  vmult_with_last_residual_norm(VectorType &dst, const VectorType &src) const;
  ...
  /**
   * Perform one step of the preconditioned iteration with the Chebyshev
   * polynomials, returning the norm of the residual used in the last
   * iteration step.  Since the Chebyshev iteration uses the residual for
   * another update step, the final residual norm is different, typically
   * smaller, than the value reported by this function.
   */
  double
  step_with_last_residual_norm(VectorType &dst, const VectorType &src) const;
  ...
  /**
   * Set the degree of the Chebyshev polynomial. The degree of the polynomial
   * gives the number of matrix-vector products to be performed for one
   * application of the step() operation. During vmult(), the method performs
   * `(degree-1)` matrix-vector products. Degree one corresponds to a damped
   * Jacobi method.
   */
  void
  set_degree(const unsigned int degree);
    ...
    if (data.eig_cg_n_iterations > 0)
      {
        ...
        // constrained). If that is the case, there is nothing we can do
        // because in both of the two methods below, we either solve a linear
        // system with temp_vector1 as the right hand side, or use it as a
...
namespace internal
{
  namespace PreconditionChebyshevImplementation
  {
    ...
    struct VectorUpdater
    {
            ...
            if (iteration_index == 0)
              {
                for (unsigned int i = begin; i < end_regular; i += n_lanes)
                  {
                    VectorizedArray<Number> rhs_i, diag_i;
                    rhs_i.load(rhs + i);
                    diag_i.load(matrix_diagonal_inverse + i);
                    const VectorizedArray<Number> res_i =
            ...
            else if (iteration_index == 1)
              {
                ...
                for (unsigned int i = begin; i < end_regular; i += n_lanes)
                  {
                    VectorizedArray<Number> rhs_i, sol_i, tmp_i, diag_i;
                    rhs_i.load(rhs + i);
                    diag_i.load(matrix_diagonal_inverse + i);
            ...
            else
              {
                ...
                for (unsigned int i = begin; i < end_regular; i += n_lanes)
                  {
                    VectorizedArray<Number> rhs_i, sol_i, sol_old_i, tmp_i,
                      diag_i;
                    rhs_i.load(rhs + i);
                    diag_i.load(matrix_diagonal_inverse + i);
                    sol_i.load(solution + i);
      ...
      mutable VectorizedArray<Number> sum_accumulator;
    };
...
inline double
PreconditionChebyshev<MatrixType, VectorType, PreconditionerType>::
  vmult_with_last_residual_norm(VectorType       &solution,
                                const VectorType &rhs) const
{
  return apply_internal<false>(/* zero_out_dst */ true,
                               rhs,
                               solution,
                               /* compute_norm */ true);
}
...
inline double
PreconditionChebyshev<MatrixType, VectorType, PreconditionerType>::
  step_with_last_residual_norm(VectorType       &solution,
                               const VectorType &rhs) const
{
...
template <typename MatrixType, typename VectorType, typename PreconditionerType>
inline void
PreconditionChebyshev<MatrixType, VectorType, PreconditionerType>::set_degree(
  const unsigned int degree)
{
  Assert(data.degree > 0,
         ExcMessage("The degree of the Chebyshev method must be positive."));
