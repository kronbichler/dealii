...
struct EvaluateTensorProduct
{
  ...
  operator()(const TeamHandle &team_member) const
  {
    ...
    if (type == 0)
      {
        evaluator.template value<d0, dof_to_quad, false, false>(src, tmp1);
        evaluator.template value<d1, dof_to_quad, false, false>(tmp1, tmp2);
        evaluator.template value<d2, dof_to_quad, add, false>(tmp2, dst);
      }
    if (type == 1)
      {
        evaluator.template derivative<d0, dof_to_quad, false, false>(src, tmp1);
        evaluator.template derivative<d1, dof_to_quad, false, false>(tmp1,
                                                                     tmp2);
        evaluator.template derivative<d2, dof_to_quad, add, false>(tmp2, dst);
      }
  }
