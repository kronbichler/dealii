...
// add>(in, out) against the already-established per-direction
// co_derivative<direction, dof_to_quad, add, in_place>() chain
...
struct FusedCoGradientsForward
{
  ...
  operator()(const TeamHandle &team_member) const
  {
    ...
    evaluator.template co_derivative<0, true, add, false>(
      u, Kokkos::subview(grad_ref, Kokkos::ALL, 0));
    if constexpr (dim > 1)
      evaluator.template co_derivative<1, true, add, false>(
        u, Kokkos::subview(grad_ref, Kokkos::ALL, 1));
    if constexpr (dim > 2)
      evaluator.template co_derivative<2, true, add, false>(
        u, Kokkos::subview(grad_ref, Kokkos::ALL, 2));

    // Fused
    evaluator.template co_gradient<false, add>(u, grad_fused);
  }
...
struct FusedCoGradientsBackward
{
  ...
  operator()(const TeamHandle &team_member) const
  {
    ...
    if constexpr (dim == 1)
      {
        evaluator.template co_derivative<0, false, add, false>(
          Kokkos::subview(grad_in, Kokkos::ALL, 0), values_ref);
      }
    else if constexpr (dim == 2)
      {
        evaluator.template co_derivative<1, false, add, false>(
          Kokkos::subview(grad_in, Kokkos::ALL, 1), values_ref);
        evaluator.template co_derivative<0, false, true, false>(
          Kokkos::subview(grad_in, Kokkos::ALL, 0), values_ref);
      }
    else // dim == 3
      {
        evaluator.template co_derivative<2, false, add, false>(
          Kokkos::subview(grad_in, Kokkos::ALL, 2), values_ref);
        evaluator.template co_derivative<1, false, true, false>(
          Kokkos::subview(grad_in, Kokkos::ALL, 1), values_ref);
        evaluator.template co_derivative<0, false, true, false>(
          Kokkos::subview(grad_in, Kokkos::ALL, 0), values_ref);
      }
    ...
    // Fused.
    evaluator.template co_gradient<true, add>(grad_in, values_fused);
  }
