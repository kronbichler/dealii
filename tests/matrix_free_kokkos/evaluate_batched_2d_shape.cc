...
struct EvaluateTensorProduct
{
  ...
  operator()(const TeamHandle &team_member) const
  {
    ...
    if (type == 0)
      {
        evaluator.template value<d0, dof_to_quad, false, false>(src, tmp);
        evaluator.template value<d1, dof_to_quad, add, false>(tmp, dst);
      }
    if (type == 1)
      {
        evaluator.template derivative<d0, dof_to_quad, false, false>(src, tmp);
        evaluator.template derivative<d1, dof_to_quad, add, false>(tmp, dst);
      }
  }
